...
public:
  ...
  void odometry_callback(const nav_msgs::Odometry& msg)
  {
    ...
    float dist = std::sqrt(std::pow(init_odom_x - odom_x, 2) + std::pow(init_odom_y - odom_y, 2) +
                           std::pow(init_odom_z - odom_z, 2));
  }

  void perception_callback(const utilities::stamped_markers& msg)
  {
    ...
    if (log_time_interval > 10)
    {
      ...
      std::cout << "Peception Message Count: " << perception_callback_count << " | Time duration: " << duration_in_secs
                << " | Throughput (msgs/sec): " << perception_callback_count / duration_in_secs << std::endl;
    }
